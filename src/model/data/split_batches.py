...
def split_raw_train() -> list[Path]:
    cfg.DATASET_BATCH_DIR.mkdir(parents=True, exist_ok=True)

    train_raw_path = cfg.DATASET_RAW_DIR / cfg.KAGGLE_DATASET_TRAIN_FILENAME
    ...
    for i in range(cfg.DATASET_SPLIT_COUNT):
        ...
        out = cfg.DATASET_BATCH_DIR / f"train_batch_{i}.csv"
...
def main():
    with mlflow.start_run(run_name="data_split_raw_train"):
        ...
        mlflow.log_param(
            "train_raw_input",
            str(cfg.DATASET_RAW_DIR / cfg.KAGGLE_DATASET_TRAIN_FILENAME),
        )
        mlflow.log_param("n_batches", cfg.DATASET_SPLIT_COUNT)

        df = pd.read_csv(cfg.DATASET_RAW_DIR / cfg.KAGGLE_DATASET_TRAIN_FILENAME)
        ...
        manifest = cfg.DATASET_BATCH_DIR / "train_raw_batches_manifest.txt"
